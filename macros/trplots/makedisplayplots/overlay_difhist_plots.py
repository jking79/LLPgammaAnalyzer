...
#    legend.SetHeader(layout['legtitle'], '')
    ...
    for histname, tree, infile, lego  in hist_list :
        ...
        if histname is not 'diff' : f1.append(TFile.Open(infile)) 
        else : f1.append(0)
        ...
        sl.append([])
        sle.append([])
        if histname is not 'diff' : orighist = f1[n].Get(hist) 
        else : orighist = 'diff'
        ...
        for bn in range( 1,lenmybins):
            ...
            binval = 0.0
            if histname is not 'diff' : binval = float(orighist.GetBinContent(bn)) 
            if histname is 'diff' : binval = sqrt(abs((sl[0][bn-1]*sl[0][bn-1])-(sl[1][bn-1]*sl[1][bn-1]/2)))
            else : sl[n].append(binval)
            binerr = 0.0
            if histname is not 'diff' : binerr = float(orighist.GetBinError(bn))
            if histname is 'diff' : binerr = sqrt(abs((sle[0][bn-1]*sle[0][bn-1])-(sle[1][bn-1]*sle[1][bn-1]/2)))
            else : sle[n].append(binerr)
            binmid = 0.0
            if histname is 'diff' : binmid = slm[bn-1]
            else : binmid = float(orighist.GetBinCenter(bn)) 
            if n is 0 : slm.append(binmid)
            #binmid = thebinmid[n][bn-1]  #float(orighist.GetBinCenter(bn))
            if binval > 0.0 :     
                #print('Filling for ' + lego[0:3] )
                if( lego[0:3] == 'res' ): 
