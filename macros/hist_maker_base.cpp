...
class makehists : llpganna_hist_base {
...
void makehists::llpgana_hist_maker( std::string indir, std::string infilelist, std::string outfilename ){
	...
	endJobs();
	for( int it = 0; it < nHists; it++ ){ if(hist1d[it]) hist1d[it]->Write(); if(hist2d[it]) hist2d[it]->Write(); }
