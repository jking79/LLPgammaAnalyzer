...
void runTimeFitter(const std::string & infilename, const std::string & plotconfig, const std::string & miscconfig,
		   const std::string & timefitconfig, const std::string & era, const std::string & outfiletext){
...
//--constructor----------------------------------------------------------------------
  ...
  f2DHistName="Data_Hist";
  f2DGHistName="Data_GHist";
...
//---------fitter.MakeTimeFits()------ run script substances class with <TimeFitter fitter;> then runs <fitter.MakeTimeFits();> -------------------
  ...
  //const auto & label = FitInfo.label;
  const auto & sflabel = GFitInfo.label;
  ...
  // get input hist
  const auto & hist = GFitInfo.ResultsMap["sigma"];
  // get range
  ...
  //c1->Print( "test.png" );
  //.......................................
  DumpFitInfo(GFitInfo, fOutFileText, fXBins);
...
int main ( int argc, char *argv[] ){
        ...
        if( argc != 7 ) { std::cout << "Insufficent arguments." << std::endl; }
               ...
               std::string plotconfig(argv[2]);
               std::string miscconfig(argv[3]);
               std::string timefitconfig(argv[4]);
               std::string era(argv[5]);
               std::string outfiletext(argv[6]);
