...
#    legend.SetHeader(layout['legtitle'], '')
    ...
    for histname, tree, infile1, infile2, lego, cutside in hist_list :
        ...
        #print(hist)
        #print(infile2)
        f1.append(TFile.Open(infile2))
        #print(f2)

        orighist1 = f1[n].Get(hist)
        orighist2 = f1[n+1].Get(hist)
        htitle = 'hist' + str(n)
        ...
        for bn in range( 2,lenmybins):
            binmid = float(orighist1.GetBinCenter(bn))
            sigpass = 0
            bkgfail = 0
            rint1 = float(orighist1.Integral(bn,lenmybins))
            lint1 = float(orighist1.Integral(1,bn-1))
            rval1 = float(rint1/norm1)
            lval1 = float(lint1/norm1)
            rint2 = float(orighist2.Integral(bn,lenmybins))
            lint2 = float(orighist2.Integral(1,bn-1))
            rval2 = float(rint2/norm2)
            lval2 = float(lint2/norm2)
            if( cutside == "<" ):
                sigpass = lval1
                bkgpass = lval2
            else :
                sigpass = rval1
                bkgpass = rval2
            if( 'GMSB' in lego  ) : h1[n].SetPoint(bn,binmid,sigpass)
            h1[n+1].SetPoint(bn,binmid,bkgpass)
        ...
        if( n == 1 ) : 
           ...
           h1[n+1].SetMarkerSize(msz)
        lego0 = lego + ' Sig Eff'
        if( 'GMSB' in lego  ) : legend.AddEntry(h1[n],lego0,'epl');
        lego1 = lego + ' Bkg Eff'
...
#x = [ -3.0, 3.0 ]
#x = [ -10.0, 25.0 ]
l = [ 0.75,0.225,0.925,0.375 ] # legend position top right
...
for rhname, pt in zip(rhname1,ptlist) :
  ...
  inhistlist = [[rhname,"",rfgmsbroc1,rfgmsbroc2,"GMSB",cut],[rhname,"",rfgmsbroc1,rfgmsbroc3,"JetHt",cut]]
  outname = soutname + rhname
  layout = { 'xtitle' : rhname, 'ytitle' : ytitle, 'title' : htitle, 'logx' : islogx, 'logy' : islogy, 'legtitle' : legtitle }
  dostack(inhistlist, outname, date, layout, ptitle,  ys, x1, l, t)
...
for rhname, pt in zip(rhname2,ptlist) :
  ...
  outname = soutname + rhname
  layout = { 'xtitle' : rhname, 'ytitle' : ytitle, 'title' : htitle, 'logx' : islogx, 'logy' : islogy, 'legtitle' : legtitle }
  dostack(inhistlist, outname, date, layout, ptitle,  ys, x2, l, t)
...
for rhname, pt in zip(rhname3,ptlist) :
  ...
  outname = soutname + rhname
  layout = { 'xtitle' : rhname, 'ytitle' : ytitle, 'title' : htitle, 'logx' : islogx, 'logy' : islogy, 'legtitle' : legtitle }
  dostack(inhistlist, outname, date, layout, ptitle,  ys, x3, l, t)
...
for rhname, pt in zip(rhname4,ptlist) :
  ...
  outname = soutname + rhname
  layout = { 'xtitle' : rhname, 'ytitle' : ytitle, 'title' : htitle, 'logx' : islogx, 'logy' : islogy, 'legtitle' : legtitle }
  dostack(inhistlist, outname, date, layout, ptitle,  ys, x4, l, t)


#inhistlist = [[rhname4,"",rfgmsbroc1,rfgmsbroc2,"GMSB",cut],[rhname4,"",rfgmsbroc1,rfgmsbroc3,"JetHt",cut]]
#outname = soutname + rhname4
#layout = { 'xtitle' : rhname4, 'ytitle' : ytitle, 'title' : htitle, 'logx' : islogx, 'logy' : islogy, 'legtitle' : legtitle }
#dostack(inhistlist, outname, date, layout, ptitle,  ys, x4, l, t)
