				 ...
				 for( int rhindx = 0; rhindx < nScRh; rhindx++){
		             ...
		             for ( auto rh_i = 0U; rh_i < nRecHits1; rh_i++ ){
						  ...
						  if( scrhid == id_i ){
						  	...
						  	auto rhe = useEnergy ? (*rhEnergy)[rh_i] : 1;	
						  	if( useEnergy && (*rhEnergy)[rh_i] < minRhEnergy ) continue;
		                  	...
		                  	sumXtalMiniRecTime[id_i] += Mini_t_i*rhe; 
						  	numXtalMiniRecTime[id_i] += rhe;
		                  	sumXtal2MiniRecTime[id_i] += Mini_t_i*Mini_t_i*rhe*rhe;
		
						  	if( fill_idinfo.ecal == ECAL::EB && Mini_t_i != 0.0 ){
		                  		sumXtalEtaRecTime[iEta] += Mini_t_i*rhe;
		                  		numXtalEtaRecTime[iEta] += rhe;
		                  		sumXtal2EtaRecTime[iEta] += Mini_t_i*Mini_t_i*rhe*rhe;
		
		                  		sumXtalPhiRecTime[iPhi] += Mini_t_i*rhe;
		                  		numXtalPhiRecTime[iPhi] += rhe;
		                  		sumXtal2PhiRecTime[iPhi] += Mini_t_i*Mini_t_i*rhe*rhe;
...
int main ( int argc, char *argv[] ){
        ...
        //else {
                ...
                auto outfilename = "KUCMS_QCD_R17_v16_phorhE5_ewmo_Cali.root";
